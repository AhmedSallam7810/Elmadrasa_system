        ...
        <div class="col-12">
            <div class="card card-outline card-primary">
                <div class="card-header">
                    <h3 class="card-title">
                        <i class="fas fa-user-edit mr-2"></i>{{ __('admin.edit_student') }}
                    </h3>
                </div>
                <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2 @error('class_id') is-invalid @enderror" id="class_id" name="class_id">
                                        ...
                                        @foreach($classes as $class)
                                            <option value="{{ $class->id }}" {{ old('class_id', optional($student->classes->first())->id) == $class->id ? 'selected' : '' }}>
                                                {{ $class->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2 @error('muhafez_id') is-invalid @enderror" id="muhafez_id" name="muhafez_id">
                                        ...
                                        @foreach($muhafezs as $muhafez)
                                            <option value="{{ $muhafez->id }}" {{ old('muhafez_id', $student->muhafez_id) == $muhafez->id ? 'selected' : '' }}>
                                                {{ $muhafez->name }}
                                            </option>
                                        @endforeach
                                    </select>
