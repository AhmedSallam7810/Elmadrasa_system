      ...
      <div class="table-responsive">
        <table class="table table-bordered table-striped">
          ...
          <tbody>
            ...
            <tr>
              <td>{{ $research->date->format('Y-m-d') }}</td>
              <td>{{ $research->student->name }}</td>
              <td>{{ $research->student->classes->first()->name ?? __('admin.not_assigned') }}</td>
              <td>
                @if($research->status=='good')<span class="badge badge-success">{{ __('admin.good') }}</span>
                @elseif($research->status=='average')<span class="badge badge-warning">{{ __('admin.average') }}</span>
                @else<span class="badge badge-danger">{{ __('admin.weak') }}</span>@endif
              </td>
              <td><span class="badge badge-info">{{ $research->degree }} / 10</span></td>
              <td>{{ $research->notes }}</td>
              <td>
                <a href="{{ route('admin.researches.show', $research) }}" class="btn btn-info btn-sm"><i class="fas fa-eye"></i> {{ __('admin.view') }}</a>
                <a href="{{ route('admin.researches.edit', $research) }}" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i> {{ __('admin.edit') }}</a>
                <form action="{{ route('admin.researches.destroy', $research) }}" method="POST" class="d-inline">
                  @csrf @method('DELETE')
            ...
            @empty
            <tr><td colspan="7" class="text-center">{{ __('admin.no_research_records_found') }}</td></tr>
            @endforelse
          </tbody>
        </table>
      </div>
