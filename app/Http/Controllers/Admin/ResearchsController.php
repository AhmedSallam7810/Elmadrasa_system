...
class ResearchsController extends Controller
{
    ...
    public function create()
    {
        ...
        $students = Student::with('classes')
            ...
            ->when($classId, fn($q) => $q->whereHas('classes', fn($q2) => $q2->where('school_classes.id', $classId)))
            ->orderBy('name')->get();
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'degree' => 'required|array','degree.*' => 'required|numeric|min:0|max:20',
    ...
    {
        $request->validate(['status'=>'required|in:good,average,weak','degree'=>'required|numeric|min:0|max:20','notes'=>'nullable|string|max:255']);
